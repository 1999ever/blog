...
class RepositorioEntrada {
    ...
    public static function obtener_todas_por_fecha_descendiente($conexion) {
        ...
        if (isset($conexion)) {
            try {
                $sql = "SELECT * FROM entradas WHERE activa = '1'  ORDER BY fecha DESC LIMIT 8";
                ...
                if (count($resultado, COUNT_RECURSIVE)) {
    ...
    public static function obtener_entrada_por_id($conexion, $id) {
        ...
        if (isset($conexion)) {
            try {
                $sql = "SELECT * FROM entradas WHERE id LIKE :id";
                $sentencia = $conexion->prepare($sql);
    ...
    public static function actualizar_entrada($conexion, $id, $titulo, $url, $texto, $activa) {
        ...
        if (isset($conexion)) {
            try {
                ...
                //lo correcto era: "if (count($resultado)){" pero no funciona, asi que lo hice de otra manera, espero que no me pueda traer fallas
                if (count($resultado, COUNT_RECURSIVE)) {
    ...
    public static function buscar_entradas_todos_los_campos($conexion, $termino_busqueda) {
        ...
        if (isset($conexion)) {
            try {
                ...
                $sentecia -> bindParam(':busqueda', $termino_busqueda, PDO::PARAM_STR);
                $sentecia -> execute();
    ...
    public static function buscar_entradas_por_autor($conexion, $termino_busqueda, $orden) {
        ...
        if (isset($conexion)) {
            try {
                ...
                $sentecia -> bindParam(':autor', $autor, PDO::PARAM_STR);
                $sentecia -> execute();
                $resultado = $sentecia -> fetchAll();
